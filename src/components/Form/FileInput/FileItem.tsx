...
const fileItem = tv({
  ...
  variants: {
    state: {
      ...
      error: {
        container: 'border border-error-300 bg-error-25',
...
export function FileItem({ name, size, state }: FileItemProps) {
  const { container, icon, deleteButton } = fileItem()

  return (
    <div key={name} className={container({ state })}>
